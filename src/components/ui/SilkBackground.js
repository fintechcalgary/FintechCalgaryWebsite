...
function markSilkReady() {
  const { body } = document;
  if (!body || body.classList.contains(READY_CLASS)) return;
  body.classList.add(READY_CLASS);
  body.classList.remove(BOOT_CLASS);
}
...
export default function SilkBackground() {
  ...
  useLayoutEffect(() => {
    ...
    // Upload static colour array once
    ...
    // Static packed uniforms (shape/surface/finish/transform/cursor presence)
    gl.uniform4f(uShape, 1.26, 0.28, 0.5, 0.0);
    gl.uniform4f(uSurface, 2.4, 1.05, -0.04, 0.88);
    gl.uniform4f(uFinish, 0.0, 0.0, 0.0, 0.04);
    gl.uniform4f(uTransform, 1581.0, 0.0, 0.0, 0.0);
    // Cursor off: presence = 0
    ...
    const revealAfterPresent = () => {
      if (revealed) return;
      revealed = true;
      window.clearTimeout(failOpen);
      // Wait one more frame so the compositor has presented the silk.
